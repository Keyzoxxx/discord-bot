                ...
                db.query(`SELECT * FROM kicks WHERE userID = ${user.id}`, async (err, kicks) => {
                    db.query(`SELECT * FROM mutes WHERE userID = ${user.id}`, async (err, mutes) => {
                        db.query(`SELECT * FROM warns WHERE userID = ${user.id}`, async (err, warns) => {
                            ...
                            .setColor(bot.color)
                            .setDescription(`❌ \`${user.tag}\` \*n'a aucune sanction !\*`)    
                          
                            if(bans.length <= 0 && kicks.length <= 0 && mutes.length <= 0 && warns.length <= 0) return message.reply({embeds: [Embed2]})
                            ...
                            collector.on("collect", async button => {
                                ...
                                if(button.user.id !== (message.user ? message.user.id : message.author.id)) return button.reply({embeds: [Embed3], ephemeral: true})
